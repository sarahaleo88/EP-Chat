...
import { performanceEvaluator, type PerformanceEvaluation } from '../../lib/performance-evaluator';
...
export function PerformanceMonitor({ isOpen, onClose, apiKey }: PerformanceMonitorProps) {
            ...
            <div style={{
              ...
              {optimizationStatus.activeOptimizations.length > 0 ? (
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {optimizationStatus.activeOptimizations.map((opt: string, index: number) => (
                    <span
                      key={index}
                      style={{
                        padding: '4px 8px',
                ...
                <div style={{ color: '#6b7280', fontSize: '14px' }}>
                  No active optimizations. Click "Apply Optimizations" to enable them.
                </div>
              )}
            </div>
          </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>{getStatusIcon(evaluation.targetsMet.timeToFirstToken)}</span>
                  <span style={{ fontSize: '14px' }}>
                    Time to First Token (&lt; 2s): {formatDuration(evaluation.summary.averageTimeToFirstToken)}
                  </span>
                </div>
                
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>{getStatusIcon(evaluation.targetsMet.completeResponse)}</span>
                  <span style={{ fontSize: '14px' }}>
                    Complete Response (&lt; 8s): {formatDuration(evaluation.summary.averageResponseTime)}
                  </span>
                </div>
                
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>{getStatusIcon(evaluation.targetsMet.cacheHitRate)}</span>
                  <span style={{ fontSize: '14px' }}>
                    Cache Hit Rate (&gt; 60%): {formatPercentage(evaluation.summary.cacheHitRate)}
                  </span>
                </div>
                
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <span>{getStatusIcon(evaluation.targetsMet.streamingImprovement)}</span>
                  <span style={{ fontSize: '14px' }}>
                    Streaming Active: {evaluation.targetsMet.streamingImprovement ? 'Yes' : 'No'}
                  </span>
                </div>
              </div>
            </div>
                ...
                <table style={{ width: '100%', fontSize: '12px' }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f8fafc' }}>
                      ...
                      <th style={{ padding: '8px', textAlign: 'center' }}>Cache</th>
                    </tr>
                  </thead>
                  <tbody>
                    {evaluation.testResults.map((result, index) => (
                      <tr key={index} style={{
                        borderBottom: index < evaluation.testResults.length - 1 ? '1px solid var(--border-in-light)' : 'none'
                      }}>
                        ...
                        <td style={{ padding: '8px', textAlign: 'center' }}>
                          {result.cacheHit !== undefined ? (result.cacheHit ? '💾' : '🔄') : '-'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
