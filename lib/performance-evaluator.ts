...
export class PerformanceEvaluator {
   ...
   */
  private async testApiResponseTime(
    testIndex: number
   ...
   */
  private async testStreamingPerformance(
    testIndex: number
  ): Promise<PerformanceTestResult> {
    const testMessage = this.testMessages[testIndex % this.testMessages.length];
    if (!testMessage) {
  ...
  private async testConcurrentRequests(): Promise<PerformanceTestResult> {
    ...
    try {
      ...
      return {
        ...
        success: false,
        errorType:
          error instanceof Error ? error.message : 'Concurrent test failed',
      };
    }
  }
  ...
  private analyzeResults(
    ...
    const streamingResults = testResults.filter(
      r => r.testName.includes('streaming') && r.success
    );
    ...
    const averageTimeToFirstToken =
      streamingResults.length > 0
        ? streamingResults.reduce(
            (sum, r) => sum + (r.timeToFirstToken || 0),
            0
    ...
    const streamingPerformance =
      streamingResults.length > 0
        ? streamingResults.reduce(
            (sum, r) => sum + (r.streamingChunks || 0),
            0
          ) / streamingResults.length
        : 0;

    const errorRecoveryResult = testResults.find(
      r => r.testName === 'error-recovery'
    );
    ...
    if (!targetsMet.timeToFirstToken) {
      recommendations.push(
        'Consider optimizing API connection or reducing initial payload size'
      );
    }
    if (!targetsMet.completeResponse) {
      recommendations.push(
        'Review API timeout settings and response size optimization'
      );
    }
    if (!targetsMet.cacheHitRate) {
      recommendations.push(
        'Improve cache key generation or increase cache TTL'
      );
    }
    if (errorRecoveryTime > 15000) {
      recommendations.push(
        'Optimize error handling and timeout configurations'
      );
    }
    if (successRate < 95) {
      recommendations.push(
        'Investigate and improve error handling reliability'
      );
    }

    if (recommendations.length === 0) {
      recommendations.push(
        'Performance targets are being met. Continue monitoring for consistency.'
      );
    }
